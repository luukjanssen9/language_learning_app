...
async def list_decks(user_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> list[Deck]:
    result = await db.execute(select(Deck).where(Deck.user_id == user_id).order_by(Deck.name))
    return list(result.scalars().all())


@router.get("/{deck_id}", response_model=DeckRead)
async def get_deck(
    deck_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession = Depends(get_db)
) -> Deck:
    return await get_owned_or_404(db, Deck, deck_id, user_id)


@router.patch("/{deck_id}", response_model=DeckRead)
async def update_deck(
    deck_id: uuid.UUID, user_id: uuid.UUID, payload: DeckUpdate, db: AsyncSession = Depends(get_db)
) -> Deck:
