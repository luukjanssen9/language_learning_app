...
async def create_deck(
    ...
    deck = Deck(**payload.model_dump(), user_id=current_user.id)
    db.add(deck)
    await db.commit()
...
async def list_decks(
    ...
    result = await db.execute(
        select(Deck).where(Deck.user_id == current_user.id).order_by(Deck.name)
    )
...
async def get_deck(
    ...
    return await get_owned_or_404(db, Deck, deck_id, current_user.id)
