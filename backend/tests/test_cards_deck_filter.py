...
async def _make_deck(client: AsyncClient, login_as: Callable[[str], Awaitable[None]]) -> dict:
    ...
    the course). Logs in as the new user before returning, so calls made
    right after this one act as that user.
    """
    ...
    deck = (
        await client.post(
            "/api/decks",
            json={"course_id": course["id"], "name": "Filter test deck"},
        )
    ).json()
...
async def test_list_cards_without_deck_id_returns_all(client: AsyncClient, login_as):
...
async def test_list_cards_filters_by_deck_id(client: AsyncClient, login_as):
...
async def test_list_cards_without_deck_id_excludes_other_users_cards(
...
async def test_get_card_from_another_users_deck_is_403(client: AsyncClient, login_as):
    deck_a = await _make_deck(client, login_as)
    card_a = await _make_card(client, deck_a["id"], deck_a["_course_id"])
    deck_b = await _make_deck(client, login_as)

    await login_as(deck_b["user_id"])
    resp = await client.get(f"/api/cards/{card_a['id']}")
    assert resp.status_code == 403


async def test_list_cards_wrong_deck_owner_is_403(client: AsyncClient, login_as):
    deck_a = await _make_deck(client, login_as)
    deck_b = await _make_deck(client, login_as)
